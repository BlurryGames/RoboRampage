...
func _process(delta: float) -> void:
	if Input.is_action_pressed("aim"):
		...
		smooth_camera.fov = lerpf(smooth_camera.fov, smooth_camera_fov, 30.0 * delta)
		weapon_camera.fov = lerpf(weapon_camera.fov, weapon_camera_fov, 30.0 * delta)
